...
def test_optimize(optimize):
    ...
    logits, optimizer, cross_entropy_loss = optimize(layers_output, label_image, learning_rate, num_classes)

    _assert_tensor_shape(logits, [2*3*4, num_classes], 'Logits')

    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        sess.run([optimizer], {label_image: np.arange(np.prod(shape)).reshape(shape), learning_rate: 10})
        test, loss = sess.run([layers_output, cross_entropy_loss], {label_image: np.arange(np.prod(shape)).reshape(shape)})
...
def test_train_nn(train_nn):
    ...
    label_image = tf.placeholder(tf.float32, name='label_image')
